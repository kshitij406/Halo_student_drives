    ...
    <main className="px-4 py-6 max-w-7xl w-full mx-auto text-white">
      ...
      <div className="space-y-10">
        {Object.entries(groupedByService).map(([serviceName, serviceDrivers]) => (
          <div key={serviceName}>
            <h3 className="text-2xl font-semibold mb-4 capitalize text-white">{serviceName}</h3>
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              {serviceDrivers.map((driver) => {
                ...
                return (
                  <Link key={driver.id} href={`/driver/${driver.id}`} className="group">
                    <div className="bg-[#1C1C1C] p-5 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 border border-transparent group-hover:border-yellow-400">
                      <div className="flex justify-between items-center mb-1">
                        <h4 className="font-semibold text-lg text-white">{driver.name}</h4>
